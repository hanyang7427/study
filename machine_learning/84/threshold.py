...
threshold = df[(df['ts1'] > 60) & (df['ts2'] < 20)]
threshold.plot(marker='o', color=['dodgerblue', 'orangered'],
	zorder=2)
df['ts1'].plot(color='dodgerblue', alpha=0.2, zorder=1)
df['ts2'].plot(color='orangered', alpha=0.2, zorder=1)
mp.legend()
